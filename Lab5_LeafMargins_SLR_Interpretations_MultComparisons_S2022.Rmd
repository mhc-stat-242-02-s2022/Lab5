...
**8. Find an estimate and a 95% confidence interval for the mean percent of leaves that are entire margined in forests where the mean annual temperature is 20 degrees Celsius.**
...
**10. Using the following code, create a scatterplot of the data with the estimated line overlaid on top, and lines showing the bounds of Scheffe-based confidence intervals for the means at each value of $X$ in the range of the data. BE SURE TO UNCOMMENT THE TWO GEOM_LINE layers after you have modified the code so these will show up in the plot.**

```{r}
library(lava) ## this contains the scheffe function

## create a new data frame for the mean temperatures from the data
predict_df <- data.frame(
  # variable name needs to be the same as in the original data set
  mean_annual_temp_C = leaf$mean_annual_temp_C
)

# ?scheffe
scheffe_cis <- scheffe(model=model_fit, newdata=predict_df)
head(scheffe_cis)

predict_df <- predict_df %>% mutate(
  scheffe_lwr = scheffe_cis[, 2], # pick second column of scheffe_cis
  scheffe_upr = scheffe_cis[, 3]  # pick third column of scheffe_cis
)

ggplot(data = leaf, mapping = aes(x = mean_annual_temp_C, y = pct_entire_margined)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  # geom_line(data = predict_df, mapping = aes(x = mean_annual_temp_C, y = ADD_HERE )) + # replace ADD_HERE with the lower scheffe bound
  # geom_line(data = predict_df, mapping = aes(x = mean_annual_temp_C, y = ADD_HERE )) + # replace ADD_HERE with the upper scheffe bound
  theme_bw()
